...
export async function generateStaticParams() {
  ...
  const cookies = await CookieSchema.find({category: "limited"}, { slug: 1 }).lean();
...
export async function generateMetadata(
  ...
  return {
    ...
    description: cookie.description
      ?? `Order the ${cookie.title} from Chunk’d Cookies – thick, stuffed, freshly baked cookie available for a limited time.`,
    keywords: ["drops", "limited drop", "cookie", "limited time"],
    openGraph: {
      title: `${cookie.title} | Chunk’d Cookies`,
      description:
        cookie.description
        ?? `Limited drop cookie by Chunk’d Cookies. Order before it sells out.`,
      images: [
        {
          url: cookie.images[0],
          width: 1200,
